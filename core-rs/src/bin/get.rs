...
fn main() {
    ...
    let threads_note = threads.map(|n| format!("，并发 {n}")).unwrap_or_default();
    ...
    logger.log(
        "INFO",
        &format!(
            "程序启动 v{VERSION} (build {BUILD_STAMP}){threads_note}，工作目录={}",
            dir.display()
        ),
    );
    ...
    if let Some(url) = url_arg {
        let rc = if url.eq_ignore_ascii_case("selftest") {
            run_selftest(&logger, &dir, threads);
            0
        } else {
            run_one(&logger, &url, &dir, threads)
        };
        std::process::exit(rc);
    }
    ...
    loop {
        ...
        if link.eq_ignore_ascii_case("selftest") {
            ...
            run_one(&logger, &link, &dir, threads);
        }
        println!();
    }
}
...
fn run_one(logger: &Arc<Logger>, url: &str, dir: &PathBuf, threads: Option<usize>) -> i32 {
    // 1) 配置 + 造引擎
...
fn run_selftest(logger: &Arc<Logger>, dir: &PathBuf, threads: Option<usize>) {
